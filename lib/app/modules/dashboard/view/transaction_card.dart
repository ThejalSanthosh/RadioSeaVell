...
class TransactionTable extends StatelessWidget {
            ...
            label: Container(
              width:
                  header == 'Items Details'
                      ? 250
                      : null, // FIXED: Set width for items column
              child: Text(
                header,
                style: TextStyle(
                  color: primaryColor,
  ...
  void _showEditDialog(BuildContext context, TransactionModel transaction) {
    ...
    final vehicleController = TextEditingController(
      text: transaction.vechilePlateNo,
    );
    ...
    final previousCreditController = TextEditingController(
      text: transaction.previousBalance.toString(),
    );
    final currentCreditController = TextEditingController(
      text: transaction.currentBalance.toString(),
    );
    final reasonController = TextEditingController();
    final cashController = TextEditingController(
      text: transaction.cashAmount.toString(),
    );

    final upiController = TextEditingController(
      text: transaction.upiAmount.toString(),
    );
    print("Items Length: ${transaction.items.length}");

    for (var item in transaction.items) {
      print(
        "${item.priceLabel} | ${item.price} | ${item.quantity} | ${item.amount}",
      );
    }
                ...
                (context, setState) => AlertDialog(
                  ...
                  content: Container(
                    width:
                        ...
                        0.8, // FIXED: Set dialog width
                    height:
                        MediaQuery.of(context).size.height *
                        0.7, // FIXED: Set dialog height
                    child: SingleChildScrollView(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                                ...
                                return TableRow(
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: Text(
                                        transaction.items[index].priceLabel,
                                        style: const TextStyle(
                                          color: Colors.black,
                          ...
                          SizedBox(height: 16),
                          _buildTextField(
                            previousCreditController,
                            'Previous Credit',
                            Icons.credit_score,
                          ),
                          SizedBox(height: 16),
                          _buildTextField(
                            currentCreditController,
                            'Current Credit',
                          ...
                          TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              hintText:
                                  'Please explain why this transaction needs correction',
                            ),
                          ),
                        ],
                      ),
                      ...
                      onPressed: () {
                        ...
                        try {
                          ...
                          for (int i = 0; i < itemControllers.length; i++) {
                            ...
                            if (price <= 0) {
                              throw Exception(
                                'Price for Item ${i + 1} must be greater than 0',
                              );
                            }
                            if (quantity <= 0) {
                              throw Exception(
                          ...
                          final upi =
                              double.tryParse(upiController.text) ?? 0.0;
                          final paidAmount = cash + upi;

                          final totalAmount = double.parse(
                            totalAmountController.text,
                          );
                          final previousBalance = double.parse(
                            previousCreditController.text,
                          );
                          ...
                          if (cash < 0 || upi < 0) {
                            throw Exception('Cash or UPI cannot be negative');
                          }

                          if (paidAmount > totalAmount + previousBalance) {
                            throw Exception(
                              'Paid amount cannot exceed total payable',
                            );
                          }
